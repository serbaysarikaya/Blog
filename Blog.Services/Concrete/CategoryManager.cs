...
namespace ProgrammersBlog.Services.Concrete
{
    public class CategoryManager : ICategoryService
    {
        ...
        {
            var categories = await _unitOfWork.Categories.GelAllAsync(null);
        ...
        {
            var categories = await _unitOfWork.Categories.GelAllAsync(c => !c.IsDeleted);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        {
            var categories = await _unitOfWork.Categories.GelAllAsync(c => !c.IsDeleted && c.IsActive);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        {
            var categoriesCount = await _unitOfWork.Categories.CountAsyc();
        ...
        {
            var categoriesCount = await _unitOfWork.Categories.CountAsyc(c=>!c.IsDeleted);
            if (categoriesCount > -1)
            {
                return new DataResult<int>(ResultStatus.Success, categoriesCount);
            }
